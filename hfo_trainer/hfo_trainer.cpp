...
bool HFOTrainer::initImpl(CmdLineParser& cmd_parser) {
    ...
    if(cmd_parser.failed()) {
        std::cerr
            << "hfo trainer: ***WARNING*** detected unsupported options: ";
...
void HFOTrainer::sampleAction() {
    ...
    if(world().existKickablePlayer()) { s_state = 1; }
...
void HFOTrainer::analyse() {
    ...
    if(wm.gameMode().type() == GameMode::PlayOn) {
        ...
        if(crossGoalLine(RIGHT, wm.ball().pos())) {
            ...
            for(auto p = wm.allPlayers().begin(); p != wm.allPlayers().end();
                ++p) {
                if(!(*p)->isValid()) continue;
...
void HFOTrainer::resetField() {
    ...
    doMoveBall(Vector2D(ball_x + 1.0, ball_y), Vector2D(1.0, 1.0));
    ...
    if(hfo_offense_on_ball > 0) {
        for(auto p = wm.allPlayers().begin(); p != end; ++p) {
            if((*p)->isValid() && (*p)->side() == LEFT) { offense_count++; }
        }
    ...
    for(auto p = wm.allPlayers().begin(); p != end; ++p) {
        ...
        if((*p)->side() == LEFT) {
            ...
            x                          = ball_x +
                .1 * pitch_length * (drand(0, 1, M_rng) + offset.first);
            y = ball_y +
                .1 * pitch_length * (drand(0, 1, M_rng) + offset.second);
